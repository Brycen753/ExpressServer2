...
function showPlayPage(req, res, next) {
    ...
    const guesses = reversedGuesses.map((guess) => {
        ...
        if (guess > secret) {
            return {guess: guess, result: ' - too high', className: 'high'};
        } 
        else if (guess < secret) {
            return {guess: guess, result: ' - too low', className: 'low'};
        } 
        else {
            return {guess: guess, result: ' - correct', className: 'correct'};
        }
    });
